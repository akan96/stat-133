...
```{r}
Reviews <- Rest %>%
select(name,business_id,city,state,categories,attributes) %>%
...
AvgReviews <- 
Reviews %>%
select(name, city, categories,state, attributes, stars) %>% 
group_by(name) %>%
...
merge3 <- attl[[1319]]
for (i in 1320:2008){
merge3 <- merge(merge3, attl[[i]], by = "row.names", all.x= TRUE) 
rownames(merge3) <- merge3$Row.names
merge3 <- merge3 %>% select(-Row.names)
}
```
...
merge4 <- attl[[2009]]
for (i in 2009:2751){
merge4 <- merge(merge4, attl[[i]], by = "row.names", all.x= TRUE) 
rownames(merge4) <- merge4$Row.names
merge4 <- merge4 %>% select(-Row.names)
}
```

Merging all merged tables

```{r}
merge1 <- merge1[-8,]
merge2 <- merge2[-12,]
merge2 <- merge2[,-(1367:1368)]
merge3 <- merge3[-8,]
merge4 <- merge4[-2,]
merge4 <- merge4[,-(1:2)]
merged <- cbind(merge1,merge2,merge3,merge4)
```


```{r}
satt <- merged %>% t() %>% as.data.frame()
c <- 1: nrow(satt)
satt2 <- satt[(c%%2==0), ]
```
...
```{r}
ro <- satt3 %>% filter(` Ambience: romantic`== "True") %>% summarize(avg= mean(stars), stdev= sd(stars))
int <- satt3 %>% filter(` intimate`== "True") %>% summarize(avg= mean(stars), stdev= sd(stars))
tour <- satt3 %>% filter(` touristy`== "True") %>% summarize(avg= mean(stars), stdev= sd(stars))
trendy <- satt3 %>% filter(` trendy`== "True") %>% summarize(avg= mean(stars), stdev= sd(stars))
upscale <- satt3 %>% filter(` upscale`== "True") %>% summarize(avg= mean(stars), stdev= sd(stars))
hipster <- satt3 %>% filter(` hipster`== "True") %>% summarize(avg= mean(stars), stdev= sd(stars))
